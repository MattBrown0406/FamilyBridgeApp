# Lovable prompt — FamilyBridge items 1–5 schema

Matt: paste the SQL below into Lovable (or the Supabase SQL editor Lovable uses) to apply schema/RPC/policy changes. **Do not skip this** — the app will save enabling checks and hold/slip logs only after this runs.

This agent **did not apply** the migration to live Supabase. The same SQL also lives in the repo at:

`supabase/migrations/20260813180000_familybridge_items_1_5.sql`

---

## What this changes

1. **Journey stages** — keep `considering` / `preparing` / `intervention` / `aftercare` and add:
   - `active_use` — Active use, no intervention yet
   - `early_recovery` — Early recovery at home  
   Updates `families.journey_stage` check constraint and `set_family_journey_stage` RPC.

2. **`enabling_checks` table** — persist in-flow and full “Am I Enabling?” results to the family (not a dead-end quiz). Family members can read all family checks and insert their own.

3. **Hold / slipped ritual** — reuse existing `consequence_events`. Add event types `held` and `slipped`, update the count trigger, and allow any family member (not only moderators) to INSERT a hold/slip log. This is family accountability, not GPS surveillance.

No other tables are required for items 1–5.

---

## Paste-ready Lovable / SQL

```sql
-- FamilyBridge items 1-5: journey stages, enabling checks, hold/slip ritual.
-- Apply in Lovable. Do not treat this as already applied in production.
...
- Confirm `families.journey_stage` accepts `active_use` and `early_recovery`.
- Confirm `enabling_checks` exists with RLS.
- Confirm a family member (not only a moderator) can insert `consequence_events` with `event_type = 'held'` or `'slipped'`.

Until this is applied, the UI still works: enabling checks and hold/slip logs will show a save error toast instead of writing to the database.
