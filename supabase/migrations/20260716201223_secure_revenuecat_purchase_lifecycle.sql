...
-- Preserve one record for each purchase recorded by the earlier RevenueCat path so
-- historical duplicate rows do not make the migration fail and the purchase still
-- cannot be reused.
