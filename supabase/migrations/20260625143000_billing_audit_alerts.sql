-- Billing audit alerts for silent Square web billing failures.
-- Detects cases like: Square payment completed, but no recurring subscription ID was stored.
...
ON public.billing_audit_alerts
FOR SELECT
TO authenticated
...
FOR EACH ROW
EXECUTE FUNCTION public.touch_billing_audit_alerts_updated_at();

-- Run the Square billing audit hourly. The Edge Function itself is idempotent
-- and only emails once per unique issue_type/payment_id alert.
