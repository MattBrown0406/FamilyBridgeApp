-- Update the post_meeting_checkin_message function to include Social types
...
BEGIN
  ...
  CASE NEW.meeting_type
    ...
    WHEN 'Date' THEN
      checkin_type := 'Date';
      emoji := '💕';
    WHEN 'Friendly Gathering' THEN
      checkin_type := 'Friendly Get Together';
      emoji := '👋';
    WHEN 'Group Event' THEN
      checkin_type := 'Group Event';
      emoji := '🎉';
    WHEN 'Family Event' THEN
      checkin_type := 'Family Event';
      emoji := '👨‍👩‍👧‍👦';
    ELSE
