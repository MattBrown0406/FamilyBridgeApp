...
BEGIN
  ...
  -- The unique activation code row lock serializes concurrent redemptions. A
  -- second request waits, then returns the first request's committed result.
  SELECT ac.*
    INTO v_activation
  ...
  -- Both invite-code columns are uniquely indexed. The exception block is a
  -- subtransaction, so a collision rolls back that family attempt before retrying.
